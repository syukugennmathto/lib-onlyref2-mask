...
void new_masking_arithmetic_to_boolean_highbits(int32_t* pR1,const int32_t* pR,mask_point* pD)
{
    int32_t mask = ((1 << D) - 1);
    //printf("mask is %u\n",mask);
    int32_t b[N]={0,0},pR1p[N]={0,0},sd_1[N] = {((mask>>1)+1),0};
    //printf("sd_1 is %u\n",sd_1[0]);
    
  //  printf("deep1\n");
    for(uint32_t i = 0; i < N; ++i) b[i] = (pR[i] + sd_1[i])%Q;
  //  printf("deep2\n");
    int32_t d[N]={0,0};
    pR1p[0] = b[0];
...
//    printf("deep4\n");
    //for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1p[i] >> base;}
    for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1p[i] >> 4;}
//    printf("deep5\n");
    for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1[i]%(uint32_t)Q;}
}
