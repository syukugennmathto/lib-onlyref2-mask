...
void new_masking_arithmetic_to_boolean_highbits(int32_t* pR1,const int32_t* pR,mask_point* pD)
{
    int32_t based =alpha;
    int32_t mask = ((1 << 23) - 1);
    int32_t b[N]={0,0},pR1p[N]={0,0},pR1pt[N]={0,0},sd_1[N];
    ...
    {
        pD->outside[i] = random_int32_t();  // pDをポインタ経由で逆参照してメンバーにアクセス
    ...
    for(uint32_t j = 1; j < N;++j)
    {
        ...
        for(uint32_t i = 1; i < N;++i)
        {
            pD->inside[j][i] = random_int32_t(); // pDを逆参照
            d[i] = pD->inside[j][i]; // 同様にpDを逆参照
            d[0] -= d[i];
        }
        for(uint32_t i = 0; i < N; ++i){pR1[i]=(pR1[i]^d[i]);}
        
    }
    //until Masking function

    //for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1p[i] >> 4;}
//    printf("deep5\n");
    //for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1[i]%based;}
    
    //for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1[i]%(uint32_t)44;}
}
...
void new_unsking_arithmetic_to_boolean_highbits(int32_t* pR,int32_t* pR1,mask_point pD)
{
    ...
    //for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1[i]*based;}
    //printf("1pR1[2] = %d\n",pR1[2]);
    //for(uint32_t i = 0; i < N; ++i){pR1[i] = pR1[i] << badominton;}
    //printf("2pR1[2] = %d\n",pR1[2]);
    b[0] = pR1p[0];
    for(uint32_t i = 0; i < N; ++i) b[i] = (pR[i] + sd_1[i]);
    for(uint32_t i = 1; i < N;++i)
   ...
   // printf("6pR1[4] = %d\n",pR1[4]);
    ...
    for(uint32_t i = 0; i < N; ++i){pR[i]=new_masking_decompose(pR1pt[i],pR1[i]);}
