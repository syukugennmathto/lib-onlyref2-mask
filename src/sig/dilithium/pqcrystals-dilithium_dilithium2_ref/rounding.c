...
unsigned char masking_arithmetic_make_hint(uint32_t* z, uint32_t* r, uint32_t base)
{
...
unsigned char num_masking_arithmetic_make_hint(uint32_t z, uint32_t r, uint32_t base)
{
...
* Returns corrected high bits.
**************************************************/
uint32_t masking_use_hint(uint32_t a, unsigned char hint) {
    uint32_t a0, a1;

    // コード2のdecompose関数を呼び出してa0とa1を計算
    a1 = decompose(&a0, a);
    masking_arithmetic_to_boolean_decompose(uint32_t* pR1, uint32_t* pR0, uint32_t *pR,alpha);
