...
rej:
    ...
    for (size_t i = 0; i < K; i++) {
        for (size_t j = 0; j < N; j++) {
            h.vec[i].coeffs[j] = num_masking_arithmetic_makeint(w4.vec[i].coeffs[j],w5.vec[i].coeffs[j], alpha);
        }
    }
    ...
    for(uint32_t i = 0; i < SEEDBYTES; i++){if(cp2.coeffs[i] != cp1.coeffs[i]){ret = 4;goto rej;}}
...
/*
  ...
  /* Matrix-vector multiplication; compute Az - c2^dt1 in line27 */
    ...
    //for(int i = 0; i < SEEDBYTES; i++){if(c[i] != cp[i]){kl = 496 - i;}}if(kl > 1){return kl;}
    /*times 2^D*/
    polyveck_shiftl(&t1);
    polyveck_ntt(&t1);
   ...
   for (uint32_t i = 0; i < SEEDBYTES; i++) {cp2.coeffs[i] = cp2.coeffs[i]%2;}
    ...
    for(uint32_t i = 0; i < SEEDBYTES; i++){if(cp2.coeffs[i] != cp1.coeffs[i]){rc = rc + i;}}
